...
export class BarChart extends BaseChart {
    ...
    public chartDatasets: Array<any>;
      ...
      static parser = function(data: any): BarChart {
          if(data.type !== 'Bar') {
              throw Error('Chart type is not Bar');
          }
          if(!data.name) {
              throw Error('Could not create chart name: Name is not set')
          }

          return new BarChart(data.name, data.sizeX, data.sizeY, data.colour, data.telemetryType, data.requestType, data.timespan, data.interval, data.aggregation, data.segment);
      };
      ...
      constructor(name: string, sizeX: number, sizeY: number, chartColour: string, telemetryType: string, requestType: string, timespan: string, interval: string, aggregation: string, segment: string) {
          super(name, 'Bar Chart', ChartType.Bar, null, sizeX, sizeY, chartColour);
